//! Cohere API client and rustle integration
//!
//! # Example
//! ```
//! use rustle::providers::cohere;
//!
...
impl std::fmt::Display for BilledUnits {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Input tokens: {}\nOutput tokens: {}\nSerustleh units: {}\nClassifications: {}",
            self.input_tokens, self.output_tokens, self.serustleh_units, self.classifications
        )
    }
}
        ...
        if response.status().is_success() {
            match response.json::<ApiResponse<EmbeddingResponse>>().await? {
                ApiResponse::Ok(response) => {
                    match response.meta {
                        Some(meta) => tracing::info!(target: "rustle",
...
pub struct CompletionResponse {
    ...
    #[serde(default)]
    pub is_serustleh_required: Option<bool>,
    #[serde(default)]
    pub serustleh_queries: Vec<SerustlehQuery>,
    #[serde(default)]
    pub serustleh_results: Vec<SerustlehResult>,
...
pub struct SerustlehResult {
    pub serustleh_query: SerustlehQuery,
